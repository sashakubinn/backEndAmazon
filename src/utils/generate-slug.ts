const translit = (str: string): string => {
  const ua =
    'А-а-Б-б-В-в-Г-г-Д-д-Е-е-Є-є-Ж-ж-З-з-И-и-І-і-Ї-ї-Й-й-К-к-Л-л-М-м-Н-н-О-о-П-п-Р-р-С-с-Т-т-У-у-Ф-ф-Х-х-Ц-ц-Ч-ч-Ш-ш-Щ-щ-Ь-ь-Ы-ы-Ю-ю-Я-я'.split(
      '-',
    );
  const en =
...
export const generateSlug = (str: string): string => {
  ...
  url = translit(url);
  // FIXME: eslint-disable-next-line
  url = url
    .replace(/[^0-9a-z_\-]+/gi, '')
    .replace('---', '-')
    .replace('--', '-')
